...
def fetch_top_parts(limit: int = 5) -> pd.DataFrame:
    pipeline = [
        {"$unwind": "$carParts"},
        {"$group": {"_id": "$carParts.title", "count": {"$sum": 1}}},
        {"$sort": {"count": -1}},
        {"$limit": limit}
    ]
    res = list(suggestions_col.aggregate(pipeline))
    ...
    df = pd.DataFrame(res).rename(columns={"_id": "part"})
    return df
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                ...
                html.H3("Common Issues and Parts", style={
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            children=[
                                html.H4("Top Selling Parts (Most Clicked)", style={
                                        "marginTop": 0}),
                                dcc.Graph(
                                    id="top-parts-chart", style={"height": "360px", "width": "100%"})
                            ]
                        )
                    ]
                ),
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            children=[
                                html.H4("Diagnostic Session Completion Rate", style={
                                        "marginTop": 0}),
                ...
                html.Div(
                    style={"marginTop": "40px", "backgroundColor": "#FFFFFF",
                           "padding": "20px", "borderRadius": "8px"},
                    children=[
                        html.H4("Avg. Diagnosis Time by Month",
                                style={"marginTop": 0}),
...
def update_dashboard(n):
    ...
    if df_issues.empty:
        ...
        fig_issues = px.bar(
            ...
            labels={"count": "Occurrences", "issue": "Issue"}, height=360
        )
    ...
    if df_avg.empty:
        ...
        fig_avg.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                              plot_bgcolor="rgba(0,0,0,0)")
    else:
        fig_avg = px.line(
            df_avg, x="day", y="avg_steps",
            labels={"day": "", "avg_steps": "Avg Steps"}, height=360
        )
    ...
    if df_comp.empty:
        ...
        fig_comp = px.bar(
            ...
            labels={"status": "", "count": "Count"}, height=360
        )
    ...
    # Top Selling Parts
    df_parts = fetch_top_parts()
    if df_parts.empty:
        fig_parts = px.bar(title="No data")
    else:
        fig_parts = px.bar(df_parts, x="count", y="part", orientation="h",
                           labels={"count": "Occurrences", "part": "Part"}, height=360)
        fig_parts.update_traces(marker_color="#E8E8E8", marker_line_width=0)
        fig_parts.update_layout(yaxis={"autorange": "reversed"}, bargap=0.2,
                                margin={"l": 140, "r": 20, "t": 20, "b": 20},
                                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
        ...
        for i, row in df_parts.iterrows():
            ...
            center = y1 - span*(i+0.5)
            half = span*0.4
            fig_parts.add_shape(type="line", x0=cnt, x1=cnt,
                                y0=center-half, y1=center+half,
                                xref="x", yref="paper",
                                line=dict(color="#EC6936", width=3))
    ...
    df["label"] = df["hour"].apply(lambda h: f"{(h % 12) or 12} {'AM' if h < 12 else 'PM'}")
